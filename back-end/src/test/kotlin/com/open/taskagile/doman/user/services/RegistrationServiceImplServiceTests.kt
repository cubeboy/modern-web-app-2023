...
package com.open.taskagile.doman.user.services
...
import com.open.taskagile.domain.application.domain.user.services.RegistrationServiceImpl
...
class RegistrationServiceImplServiceTests {
  ...
  fun setUp() {
    ...
    registrationService = RegistrationServiceImpl(userRepository, eventPublisher, userNotifier)
  }
  ...
  fun `중복된 Username 등록 오류`() {
    ...
    val id = registrationService.register(command)
    id.test()
      .expectSubscription()
      .expectErrorMatches {
    ...
    verify { userRepository.register(any(), any(), any()) wasNot called }
  }
  ...
  fun `사용자 등록 성공`() {
    ...
    verifySequence {
      ...
      userNotifier.notify(userId, NotificationType.USER_REGISTERED )
    }
  }
}
